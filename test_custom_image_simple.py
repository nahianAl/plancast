#!/usr/bin/env python3
"""
Simple Custom Image 3D Generation Test
This script tests your own floor plan image without interactive input.
"""
    ...
    def test_with_custom_image(self, image_path: str, room_dimensions: Dict[str, Any] = None):
        ...
        try:
            ...
            with Image.open(image_path) as img:
                ...
                # Process with CubiCasa
                cubicasa_output = self.cubicasa_service.process_image(img_bytes, "test_job")
                ...
                print(f"   🏠 Rooms detected: {len(cubicasa_output.room_bounding_boxes)}")
                print(f"   🧱 Walls detected: {len(cubicasa_output.wall_coordinates)}")
                print(f"   🚪 Doors detected: {len(cubicasa_output.door_coordinates)}")
                print(f"   🪟 Windows detected: {len(cubicasa_output.window_coordinates)}")
                ...
                for i, suggestion in enumerate(suggestions):
                    print(f"   {i+1}. {suggestion['room_name']}: {suggestion['reason']} (Priority: {suggestion['priority']})")
                ...
                if room_dimensions:
                    ...
                    print(f"\n📏 Using provided dimensions: {room_width}' x {room_length}' for room: {selected_room['room_name'] if selected_room else 'Unknown'}")
                else:
                    # Use defaults
                    print(f"\n📏 Using default dimensions: 20' x 15'")
                    selected_room = suggestions[0] if suggestions else None
                    room_width, room_length = 20, 15
                
                # Scale coordinates
                if selected_room:
                    print(f"\n📏 Scaling coordinates using {selected_room['room_name']} as reference...")
                    # First calculate the scale factor
                    scale_reference = self.coordinate_scaler.calculate_scale_factor(
                        cubicasa_output,
                        selected_room['room_name'],
                        'width',  # Use width for scaling
                        room_width
                    )
                    print(f"✅ Scale factor calculated: {scale_reference.scale_factor:.4f} pixels/foot")
                    
                    # Then convert coordinates to feet
                    scaled_coords = self.coordinate_scaler.convert_coordinates_to_feet(
                        cubicasa_output,
                        scale_reference
                    )
                    print(f"✅ Scaling completed with factor: {scaled_coords.scale_reference.scale_factor:.4f}")
                else:
...
def main():
    ...
    # Use test_image.jpg by default
    image_path = "test_image.jpg"
    
    if not os.path.exists(image_path):
        print(f"❌ Image not found: {image_path}")
        print("Please place a floor plan image named 'test_image.jpg' in the current directory")
        return
    ...
    print(f"📐 Using default dimensions: {room_dimensions['width']}' x {room_dimensions['length']}'")
